...
- **Failure recovery (reset-to-queue):** An hourly job (**`retryFailedAudits`**) scans **every** **`Week N`** tab in the workbook for failed audit text (`GEMINI REJECTED`, `PENDING API RETRY`, etc.) and **resets those cells to the pending placeholder**. It does **not** call Gemini inline. The **10-minute** queue picks up each row in isolation—no multi-retry pile-up in a single run.
...
| File | Description |
|------|-------------|
| `Main.js` | `onFormSubmit`, `doPost` (Telegram), `createTriggers`, **`processPendingAudits`**, **`retryFailedAudits`** (hourly sweep of all **`Week N`** tabs, reset-to-queue). |
